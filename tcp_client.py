...
def main():
    ...
    local_addr = client.getsockname()

    init_data = len(blocks).to_bytes(4, 'big')
    init_header = struct.pack('!II', 1, len(init_data))
    log_event('CLIENT', f"send INIT local={local_addr} server=({server_ip},{server_port}) count={len(blocks)} Lmin={Lmin} Lmax={Lmax} seed={seed} lengths={chunk_lengths}")
    ...
    if agree_header:
        ...
        if type_field == 2:
            log_event('CLIENT', f"recv AGREE local={local_addr}")
            print("收到agree报文")
    ...
    for idx, block in enumerate(blocks):
        ...
        log_event('CLIENT', f"send REQUEST idx={idx} len={len(block_data)} local={local_addr}")
        ...
        log_event('CLIENT', f"recv ANSWER idx={idx} len={len(reversed_data)} local={local_addr}")
    ...
    log_event('CLIENT', f"write reversed file {output_path.name} len={len(output_content)} local={local_addr}")

    log_event('CLIENT', f"close connection local={local_addr}")
    client.close()
    print("传输完成")
